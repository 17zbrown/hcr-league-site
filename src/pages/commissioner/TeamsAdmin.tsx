...
function TeamRow({ team, onChange }: { team: Team; onChange: () => void }) {
  ...
  const save = async () => {
    setBusy(true)
    await supabase.from('teams').update({ name, number, car, class_id: classId }).eq('id', team.id)
    setBusy(false)
    setSaved(true)
  ...
  return (
    <div className="grid items-center gap-2 rounded-xl border border-[var(--color-line)] bg-[var(--color-paper)] p-3 md:grid-cols-[70px_1.5fr_1.5fr_110px_auto_auto]">
