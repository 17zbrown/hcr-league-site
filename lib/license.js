/* Shared HCR racing-license computation (public site + admin).
   The class is EARNED from the driver's league record — qualifying + pace,
   race results, incident cleanliness — weighted by experience so it climbs
   only as a real record builds. A league-assigned class (manual override)
   is absolute and takes precedence. */
...
export const LICENSE_GATE = { Platinum: 10, Gold: 7, Silver: 4 };
...
export function driverLicense(driver, events) {
  ...
  let pace = null, results = null, safety = null, rating = null, incPerRace = null;
  if (starts) {
    ...
    const conf = Math.min(1, starts / 10);            // full weight only at 10 starts
    rating = Math.round(perf * conf + 38 * (1 - conf)); // low floor: one race can't vault a tier
  }

  /* manual assignment is absolute */
  if (override) {
    return { tier: override, color: tierColor(override), source: "manual", override,
      rating, pace, results, safety, incPerRace, starts, provisional: false };
  }
  if (!starts) return { tier: "Unranked", color: "#6b7686", source: "none", override: null,
    rating: null, pace: null, results: null, safety: null, incPerRace: null, starts: 0, provisional: true };
  ...
  while (t.name !== "Bronze" && starts < (LICENSE_GATE[t.name] || 0)) {
    t = LICENSE_TIERS[LICENSE_TIERS.indexOf(t) + 1];   // not enough starts — hold the tier below
  }
  return { tier: t.name, color: t.color, source: "league", override: null,
    rating, pace, results, safety, incPerRace, starts, provisional: starts < 5 };
}
