...
export async function raceDayOutlook(lat, lon, dateStr) {
  ...
  const data = await fetchJson(
    `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&start_date=${dateStr}&end_date=${dateStr}` +
    ...
    `&daily=temperature_2m_max,temperature_2m_min&${UNITS}&timezone=${TZ}`, 4800);
  const h = data?.hourly;
  if (!h?.time) return { available: false, reason: "unavailable", daysAhead };
  let idx = h.time.indexOf(`${dateStr}T20:00`);
  if (idx < 0) idx = Math.min(20, h.time.length - 1);
  const from = Math.max(0, idx - 4), to = Math.min(h.time.length - 1, idx + 4);
  const hourly = [];
  ...
  return {
    ...
    at: {
      ...
      code: h.weather_code?.[idx], highF: r0(d.temperature_2m_max?.[0]), lowF: r0(d.temperature_2m_min?.[0]),
    },
