...
export async function fetchLeagueData(seasonId) {
  ...
  const sb = createClient(URL, ANON, { auth: { persistSession: false }, global: { fetch: (u, i) => fetch(u, { ...i, cache: "no-store" }) } });
  ...
  const shapedDrivers = drivers
    ...
    .map((d) => {
      ...
      return { id: d.id, name: d.name, country: d.country || "", car: t.car || "", num: t.number, cls: t.class_id, teamId: t.id, custId: d.iracing_custid || "", irating: d.irating || null, licenseOverride: d.license_override || null, pointsAdjust: d.points_adjust || 0 };
    });
  ...
  const shapedEvents = events.map((ev) => {
    ...
    const evResults = (resByEvent[ev.id] || [])
      .map((r) => {
        ...
        return {
          ...
          drivers: driversText, nat: "", car: r.car || (t && t.car) || "",
    ...
    return {
      id: ev.id, round: ev.round, name: ev.name || "",
      track: track?.name || "TBA", location: track?.location || "",
      lat: track?.lat != null ? Number(track.lat) : null, lon: track?.lon != null ? Number(track.lon) : null,
      durationMin: ev.duration_min != null ? Math.round(Number(ev.duration_min)) : Math.round((Number(ev.duration_h) || 0) * 60),
      durationH: (ev.duration_min != null ? Number(ev.duration_min) : (Number(ev.duration_h) || 0) * 60) / 60,
      ...
      minDrivers: ev.min_drivers, maxDrivers: ev.max_drivers, notes: ev.notes || "", report: ev.report || "",
      sessions: (sessByEvent[ev.id] || []).map((s) => ({ type: s.type, start: s.start, durMin: s.dur_min })),
      weather: (wxByEvent[ev.id] || []).map((w) => ({ atHour: Number(w.at_hour), air: Number(w.air_f), clouds: w.clouds, sky: w.sky, precip: w.precip, wind: Number(w.wind_mph), humidity: w.humidity })),
  ...
  results.forEach((r) => {
    ...
    if (!cur || sec < cur.sec) {
      tr.perClass[cls] = {
        ...
        drivers: drivers_text, car: r.car || (t && t.car) || "", driverIds,
  ...
  return {
    ...
    teams: teams.map((t) => ({ id: t.id, name: t.name, number: t.number, car: t.car, cls: t.class_id, pointsAdjust: t.points_adjust || 0 })),
