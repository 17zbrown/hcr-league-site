...
// `apikey` with no Authorization header, which would make any such path world-callable.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // moved; a scheduled run gets the same shape with the identities left out,
    // because nothing on that end is reading them.
